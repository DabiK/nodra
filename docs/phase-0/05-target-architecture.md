...
3. Domaine indépendant de Codex, OpenCode et futurs protocoles provider.
4. Socle provider commun et capacités optionnelles interrogables par le produit.
5. Codex et OpenCode comme premiers adaptateurs; ajout futur de Claude ou autre sans refonte métier.
...
- **DEC-V** — Codex est intégré par app-server JSON-RPC; OpenCode par son serveur local HTTP/OpenAPI. Aucun adaptateur ne parse une sortie CLI.

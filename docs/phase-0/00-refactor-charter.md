...
- **DEC-V — Exécution multi-provider extensible.** Codex et OpenCode sont les providers initiaux derrière un contrat d'adaptateur stable. Codex utilise app-server JSON-RPC; OpenCode utilise son serveur local HTTP/OpenAPI, sans parsing de CLI. Le domaine reste indépendant de leurs protocoles. Le socle produit requis est disponibilité, modèles, start, événements, stop, résultat, reprise, queue, steer, usage et pièces jointes; chaque adaptateur déclare sa faisabilité réelle, sans simulation ni fallback. L'ajout futur de Claude ou d'autres providers doit rester possible sans refonte du domaine.

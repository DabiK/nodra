...
export class SqliteGateRepository implements GateRepository {
    ...
    try {
      this.database.orm.transaction((tx) => {
        ...
        for (const evidenceId of evaluation.evidenceIds) {
          ...
          tx.insert(gateEvaluationEvidence).values({ evaluationId: evaluation.id, evidenceId, role: "observation" }).run();
        }
  ...
  async listPassedGitEvaluations(runId: Parameters<GateRepository["listPassedGitEvaluations"]>[0]) {
    try {
      const rows = this.database.orm.select({ evaluation: gateEvaluations, criteriaJson: gateDefinitions.criteriaJson }).from(gateEvaluations).innerJoin(gateBindings, eq(gateBindings.id, gateEvaluations.gateBindingId)).innerJoin(gateDefinitions, eq(gateDefinitions.id, gateBindings.gateId)).where(and(eq(gateEvaluations.runId, runId), eq(gateEvaluations.state, "passed"))).all().filter((row) => (JSON.parse(row.criteriaJson) as Record<string, unknown>).requiresGit === true).map((row) => row.evaluation);
      return this.hydrate(rows);
    } catch (error) { throw translateSqliteError(error); }
  }
  ...
  async override(input: Parameters<GateRepository["override"]>[0]) {
    try {
      return this.database.orm.transaction((tx) => {
        ...
        if (!approval || approval.state !== "approved" || (approval.expiresAt && approval.expiresAt <= input.context.occurredAt)) throw new DomainError("An approved, unexpired approval is required", "APPROVAL_REQUIRED");
        if (approval.runId !== row.evaluation.runId && approval.missionId !== row.missionId) throw new DomainError("Approval target does not match the gate evaluation", "APPROVAL_TARGET_MISMATCH");
  ...
  private hydrate(rows: Array<typeof gateEvaluations.$inferSelect>): GateEvaluationRecord[] {
    ...
    return rows.map((row) => this.record(row, links.filter((link) => link.evaluationId === row.id).map((link) => link.evidenceId)));
  }
  ...
  private record(row: typeof gateEvaluations.$inferSelect, ids: readonly string[]): GateEvaluationRecord {
    if (!row.runId) throw new DomainError("Gate evaluation is not attached to a run", "GATE_EVALUATION_INVALID");
    return { id: toId(row.id), gateBindingId: toId(row.gateBindingId), runId: toId(row.runId), evaluatorId: row.evaluatorId, evaluatorVersion: row.evaluatorVersion, state: row.state, evaluatedAt: row.evaluatedAt, staleAt: row.staleAt, rationale: row.rationale ?? "", evidenceIds: ids.map(toId) };
  }

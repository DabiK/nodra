...
describe("I4 API", () => {
  ...
  it("exposes the complete structured proof and human acceptance flow", async () => {
    ...
    await writeFile(join(workspace, "tracked"), "changed"); const stale = await request(app.getHttpServer()).post("/api/gates/runs/run-api-i4/refresh-staleness").send({ commandId: "api-stale" }).expect(201); expect(stale.body.stale).toHaveLength(1); const evaluationId = stale.body.stale[0] as string;
    ...
    const approval = await request(app.getHttpServer()).post("/api/approvals").send({ subjectType: "run", subjectId: "run-api-i4", kind: `gate_override:${evaluationId}`, commandId: "api-approval" }).expect(201);
    ...
    await request(app.getHttpServer()).get(`/api/gates/${gate.body.definition.id}/evaluations`).expect(200).expect(({ body }) => expect(body[0].id).toBe(evaluationId));
  ...
  it("rejects an incomplete expected-evidence schema before persisting an immutable gate", async () => { const response = await request(app.getHttpServer()).post("/api/gates").send({ missionId: "mission-api-i4", name: "invalid", evaluatorId: "command-exit", evaluatorVersion: "1", criteriaSchemaVersion: 1, criteria: { schemaVersion: 1, expectedExitCode: 0, requiresGit: true }, expectedEvidence: { schemaVersion: 1, kind: "observation" }, commandId: "invalid-gate" }).expect(422); expect(response.body).toMatchObject({ code: "GATE_DEFINITION_INVALID", commandId: "invalid-gate" }); });
});

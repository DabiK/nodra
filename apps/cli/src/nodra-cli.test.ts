...
describe("NodraCli", () => {
  ...
  beforeEach(async () => {
    ...
    cli = new NodraCli(
      ...
      output,
      new I4Cli([new EvidenceCli(new ReadEvidence(evidence), new CollectEvidence(evidence, blobs, new LocalCommandObservationAdapter(git), git)), new GateCli(gates), new ApprovalCli(new ManageApprovals(new SqliteApprovalRepository(database))), new DeliveryCli(new ManageDelivery(new SqliteDeliveryRepository(database), gates))])
    );
  });

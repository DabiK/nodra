...
export class GateCli implements I4CliHandler {
  ...
  async execute({ command, args, context }: I4CliRequest) {
    ...
    if (command === "gate:define") { const exitIndex = args.indexOf("--expected-exit"); if (!args[0] || !args[1]) throw new DomainError("gate:define requires mission and name", "CLI_USAGE_ERROR"); const id = toId(randomUUID()); const requiresGit = args.includes("--requires-git"); return this.gates.define({ definition: { id, name: args[1], evaluatorId: "command-exit", evaluatorVersion: "1", criteriaSchemaVersion: 1, criteria: { schemaVersion: 1, expectedExitCode: exitIndex < 0 ? 0 : this.integer(args[exitIndex + 1]), requiresGit }, expectedEvidence: { schemaVersion: 1, kind: "observation", collectorId: "nodra.command", collectorVersion: "1", requiredBlobRoles: ["stdout", "stderr"], subject: { type: requiresGit ? "git-tree" : "content-digest" } }, createdAt: context.occurredAt }, missionBindingId: toId(randomUUID()), missionId: toId(args[0]), context }); }
